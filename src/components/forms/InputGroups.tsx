...
export const TextInputGroup = ({type="text",label, placeholder,name, required=true, action, defaultValue}:{label: string, placeholder:string, name:string, required?: boolean, defaultValue?: string, type:string, action?:(e:string | number) => unknown}) => {
     return (
          <div className="w-full  flex flex-col items-start gap-[5px]">
               <label className="w-full rounded-[5px] text-[1rem] font-medium text-gray-800 text-start" htmlFor={name}>{label}</label>
               <Input type={type} className="w-full" name={name} id={name} placeholder={placeholder} required={required} onChange={(e:ChangeEvent<HTMLInputElement>) => action ? action(e.target.value) : () =>{}} />
          </div>
     )
}
